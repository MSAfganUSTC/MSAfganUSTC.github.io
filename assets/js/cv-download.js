(function () {
  ...
  function createTranslatedHeader() {
    ...
    const visibleName = document.querySelector(".author__name");
    const visibleBio = document.querySelector(".author__bio");
    const emailLink = document.querySelector('.author__urls a[href^="mailto:"]');
    const websiteLink = document.querySelector('.author__urls a[href^="http"]');

    const photoSrc = visiblePhoto ? (visiblePhoto.currentSrc || visiblePhoto.src) : "/images/profile.png";
    const name = visibleName ? cleanText(visibleName.textContent) : "Muhammad Sher Afgan";
    const bio = visibleBio ? cleanText(visibleBio.textContent) : "";
    const email = emailLink ? String(emailLink.getAttribute("href") || "").replace(/^mailto:/i, "") : "msafgan@mail.ustc.edu.cn";
    const website = websiteLink && /msafganustc\.github\.io/i.test(websiteLink.href)
      ? websiteLink.href.replace(/\/$/, "")
      : "https://msafganustc.github.io";

    header.innerHTML =
      '<div class="pdf-export-header__text">' +
        '<h1>' + escapeHtml(name) + '</h1>' +
        (bio ? '<p class="pdf-export-header__bio">' + escapeHtml(bio) + '</p>' : '') +
        '<p class="pdf-export-header__contact">' + escapeHtml(email) + ' &middot; ' + escapeHtml(website) + '</p>' +
      '</div>' +
