---
permalink: /education/
author_profile: true
title: false
---

## Ph.D. Candidate in School of Cyber Science and Technology

**University of Science and Technology of China (USTC)**  
Hefei, China  
**Present**

- Current CGPA: **3.71/4.00**
- Engaged in advanced research in computer science.
- Research focus: Computer Vision, Generative AI, diffusion models, and multimodal understanding.
- Expected Ph.D. graduation: **October 2026**

## M.S. in Computer Science

**The Islamia University of Bahawalpur (IUB)**  
Bahawalpur, Pakistan  
**2016 – April 2018**

- Research-based Master's program.
- CGPA: **3.67/4.00**
- Thesis: *Efficient Crypto-Steganography Technique for Colored Image Protection.*
- Published two research papers, including a paper in **IEEE Access**.

## B.S. in Computer Science

**The Islamia University of Bahawalpur (IUB)**  
Bahawalpur, Pakistan  
**2011 – September 2015**

- CGPA: **3.92/4.00**
- Ranked **1st among 1,120 students** and awarded the **Gold Medal**.
- Final-year project: *Radiologic X-Ray Image Enhancer & Analyzer (RXIEA)* using C# and C++.
- Completed academic projects in C#, C++, Java, PHP, HTML/CSS, and JavaFX.

## Ph.D. Candidate in School of Cyber Science and Technology

**University of Science and Technology of China (USTC)**  
Hefei, China  
**Present**

- Current CGPA: **3.71/4.00**
- Engaged in advanced research in computer science.
- Research focus: Computer Vision, Generative AI, diffusion models, and multimodal understanding.
- Expected Ph.D. graduation: **October 2026**
## M.S. in Computer Science

**The Islamia University of Bahawalpur (IUB)**  
Bahawalpur, Pakistan  
**2016 – April 2018**

- Research-based Master's program.
- CGPA: **3.67/4.00**
- Thesis: *Efficient Crypto-Steganography Technique for Colored Image Protection.*
- Published two research papers, including a paper in **IEEE Access**.

## B.S. in Computer Science

**The Islamia University of Bahawalpur (IUB)**  
Bahawalpur, Pakistan  
**2011 – September 2015**

- CGPA: **3.92/4.00**
- Secured **Ranked 1st among 1,120 students and awarded the Gold Medal**.
- Final-year project: *Radiologic X-Ray Image Enhancer & Analyzer (RXIEA)* using C# and C++.
- Completed academic projects in C#, C++, Java, PHP, HTML/CSS, and JavaFX.
